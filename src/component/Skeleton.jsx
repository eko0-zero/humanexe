...
const Skeleton = forwardRef(
  ({ model, world, characterBody, mouseWorld, isDragging }, ref) => {
    ...
    // Applique un delta via quaternion pour éviter le gimbal lock
    ...
    // Recapture les quaternions au repos — appelé par Interaction après init mixer
    ...
    useImperativeHandle(ref, () => ({
      ...
      updateBones: () => {
        ...
        if (b.neck) {
          b.neck.rotation.z =
            (b.neck.userData.restZ || 0) + sp.headZ.value * 0.6;
          b.neck.rotation.x =
            (b.neck.userData.restX || 0) + sp.headX.value * 0.5;
        }

        // ── Corps ──
        if (b.spine) {
          b.spine.rotation.z =
            (b.spine.userData.restZ || 0) + sp.bodyLeanZ.value;
          b.spine.rotation.x =
            (b.spine.userData.restX || 0) + sp.bodyLeanX.value;
          b.spine.scale.y = 1 + sp.bodyStretchY.value;
        }

        // ── Bras gauche ──
        // inside-arm-topl (z~-2.04) : épaule, swing via quaternion sur Y local
        applyQuatDelta(
          b.leftArmInside,
          0,
          sp.leftArmSwing.value * 0.4,
          0,
          "YXZ",
        );
        // for-arm-topl : bras principal, swing Z + drop X
        applyQuatDelta(
          b.leftArmTop,
          sp.armsDropY.value,
          0,
    ...
    useEffect(() => {
      ...
      // Bras gauche — noms réels du GLB
      b.leftArmInside = findBone("inside-arm-topl");
      b.leftArmTop = findBone("for-arm-topl");
      ...
      // Bras droit
      b.rightArmInside = findBone("inside-arm-topr");
      b.rightArmTop = findBone("for-arm-topr");
      ...
      // Capture initiale des valeurs au repos (t=0)
      // Note : captureRestPose() sera rappelé par Interaction après le mixer init
      for (const key in b) {
        if (!b[key]) continue;
        b[key].userData.restY = b[key].position.y;
        b[key].userData.restZ = b[key].rotation.z;
